...
class _Worker(QObject):
    ...
    gdl_missing   = pyqtSignal()           # binary not found — consumer should invalidate + re-show wizard
    ...
    def run(self):
        try:
            ...
            # Binary does not exist at the configured path.
            self.gdl_missing.emit()
            self.log_line.emit("error", "gallery-dl was not found. Please reinstall or locate it.")
            self.finished.emit(127)   # conventional command-not-found exit code
...
class DownloadJob(QObject):
    ...
    finished         = pyqtSignal()
    gdl_missing      = pyqtSignal()           # binary not found — connect to invalidate_gdl_cmd() + re-show wizard
    ...
    def __init__(self, url: str, opts: DownloadOptions, gdl_cmd: str = "gallery-dl"):
        ...
        self._worker.gdl_missing.connect(self.gdl_missing)
